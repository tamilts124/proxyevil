...
- [x] config.py: schema versioning / migration path — status: tested
  - impl: `_MIGRATIONS` registry (version → migration fn) + `_migrate_config()` applies steps sequentially from a config's `schema_version` (missing = treated as v0) up to current `SCHEMA_VERSION`, with cycle-detection so a migration that forgets to bump the version can't hang the loader. `_migrate_v0_to_v1` is the first (mostly a no-op stamp, since v0→v1 field renames were already handled ad-hoc) and serves as the template for future breaking changes. Wired into `load_config()` before validation. 6 new tests (20 total in test_config.py).

## Round 4: complete — all items tested.
