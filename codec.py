...
def decompress(data: bytes, encoding: str) -> Tuple[bytes, bool]:
    """Decompress *data* according to *encoding*.

    Returns ``(bytes, did_decompress)``.  Callers must check the flag:
    if it is ``False`` the original bytes are returned unchanged and the
    Content-Encoding header must be left intact.
    """
    enc = encoding.lower()
    try:
        if enc == "gzip":
