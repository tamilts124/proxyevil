...
def _is_ip_or_localhost(host: str) -> bool:
    ...
    if host == "localhost":
        return True
    # Fix E: skip real .local mDNS addresses — registering printer.local would
    # produce printer.local.local as the fake, which is confusing and wrong.
...
class DomainAliasAddon:
    ...
    def notify_reload(self):
        self._cfg_gen += 1
        # Reset cap counter to match the alias_map state: after reload() the
        # dynamic aliases are gone (config-only aliases were reloaded), so the
        # counter must go back to zero or it would allow max_dynamic * N_reloads
        # total registrations across successive hot-reloads.
        # alias_map.reload() is called before notify_reload() by both watcher.py
        # and sidecar._handle_reload(), so mapping no longer contains dynamic
        # aliases at this point — resetting to 0 is correct.
        self._dynamic_alias_count = 0
        self._init_access_log()
    ...
    def request(self, flow: mhttp.HTTPFlow):
        ...
        if flow.request.content:
            ...
            if any(t in ct for t in ("json", "form", "text", "xml")):
                # Guard against oversized request bodies before decompressing.
                if len(flow.request.content) > self.body_size_limit:
                    log.debug(f"[REQ body] skipping {len(flow.request.content) // 1024}KB body (over limit)")
                else:
                    try:
    ...
    def _rewrite_body(self, flow: mhttp.HTTPFlow, rw: dict,
        ...
        # Check compressed size first as a cheap pre-filter, then re-check
        # after decompression — the real guard is on decompressed bytes because
        # a 1 MB gzip body can expand to 50 MB and max_body_bytes is a RAM limit.
        ...
        if not spoof_enabled:
            if did_decompress or not enc or enc == "identity":
                if len(decompressed) <= _PRECHECK_LIMIT:
                    ...
                    # For large bodies we still need case-insensitive search.
                    # Lowercasing 10 MB costs ~10 ms which is acceptable here;
                    # it's cheaper than running the full regex over a body with
                    # no real-domain references at all.
                    dec_lower = decompressed.lower()
                    if not any(n in dec_lower for n in self.aliases._real_needles):
                        return 0
        ...
        if spoof_enabled:
            ...
            # Build the batch to register, respecting the cap, before taking any lock.
            # Snapshot both maps once to avoid 2*N lock acquisitions inside the loop.
            existing_f2r = self.aliases.mapping                         # takes lock once
            existing_r2f = {v: k for k, v in existing_f2r.items()}     # cheap inversion
